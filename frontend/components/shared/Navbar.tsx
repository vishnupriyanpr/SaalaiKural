...
import { Notification } from "@/lib/seedData";
...
export default function Navbar({ portal, userId, userName, userPoints }: NavbarProps) {
    ...
    const loadNotifs = async () => {
      try {
        ...
        setNotifications(all.filter((n) =>
          portal === "admin"
            ? n.target_role === "admin" || n.target_role === "all"
            : (n.target_role === "civilian" && n.target_id === userId) || n.target_role === "all"
        ));
      } catch {}
    };
    loadNotifs();

    const unsub = db.subscribe(async (state) => {
      ...
      if (portal === "civilian" && userId) {
        const civ = state.civilians.find((c) => c.id === userId);
        if (civ) {
          setPoints((prev) => {
            if (civ.points_total !== prev) {
              setPrevPoints(prev);
              setPointsFlash(true);
              setTimeout(() => setPointsFlash(false), 800);
            }
            return civ.points_total;
          });
        }
      }
    });
  ...
  const logout = () => {
    localStorage.removeItem("roadguard_session");
    router.push("/login");
  };
        ...
        <div className="flex items-center space-x-2 md:space-x-3">
          ...
          {portal === "civilian" && (
            <motion.div
              ...
              className="bg-primary/10 border border-primary/20 px-3 py-1 rounded-full flex items-center space-x-1.5"
            >
              <motion.span animate={{ rotate: [0, 15, -10, 0] }} transition={{ duration: 0.8, repeat: Infinity, repeatDelay: 3 }}>
                <Award className="w-4 h-4 text-primary" />
              </motion.span>
              <span className="font-mono text-xs font-bold text-primary">{points.toLocaleString()} PTS</span>
            </motion.div>
          )}
            ...
            <AnimatePresence>
              {showNotifs && (
                <motion.div
                  ...
                  className="absolute right-0 mt-3 w-80 max-h-96 overflow-y-auto rounded-xl bg-white border border-slate-200 shadow-2xl p-4 z-50"
                >
                  <div className="flex justify-between items-center mb-3 pb-2 border-b border-slate-100">
                    <span className="font-bold text-sm text-slate-800">Notifications</span>
                    {unread > 0 && <span className="text-xs text-primary font-mono">{unread} New</span>}
                  </div>
                  <motion.div
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((notif) => (
                        ...
                        >
                          ...
                          <p className="text-[11px] leading-relaxed">{notif.body}</p>
                        </motion.div>
                      ))
                    )}
                  </motion.div>
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              ...
              className="lg:hidden absolute top-[61px] left-0 w-full bg-white border-b border-slate-200 shadow-xl overflow-hidden z-40"
            >
              <motion.div
                variants={{ show: { transition: { staggerChildren: 0.05 } } }}
                initial="hidden"
