...
import { Complaint, CivilianUser, Worker } from "@/lib/seedData";
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-bg-light dark:bg-bg-dark text-slate-800 dark:text-slate-100 flex flex-col items-center justify-center">
        ...
        <p className="text-xs text-slate-450 mt-2 font-mono">Loading complaints database...</p>
      </div>
    }>
      <ComplaintsContent />
    </Suspense>
...
function ComplaintsContent() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const wrks = await db.getWorkers();
      setComplaints(comps);
      setCivilians(civs);
      setWorkers(wrks);
    };
  ...
  useEffect(() => {
    if (activeParamId && complaints.length > 0) {
      ...
      if (found) {
        ...
        setOverridePoints(found.ai_classification.recommended_points);
        if (found.worker_id) {
  ...
  const handleAssignWorker = async () => {
    ...
    try {
      ...
      await db.createNotification({
        target_role: "civilian",
        target_id: selectedComp.civilian_id,
        title: "Worker Dispatched! 🛠️",
        body: `Work has been assigned to contractor '${worker?.name}' for your report of '${selectedComp.title}'.`,
        type: "complaint_update"
      });
  ...
  const handleCloseCase = async () => {
    ...
    try {
      await db.updateComplaint(selectedComp.id, {
        status: "resolved",
        budget_actual: Math.round(selectedComp.budget_estimated * 0.95), // 5% cost optimization
      });
  ...
  const handleBulkClose = async () => {
    ...
    try {
      for (const id of selectedIds) {
        ...
        await db.updateComplaint(id, {
          status: "resolved",
          budget_actual: comp ? Math.round(comp.budget_estimated * 0.95) : 10000,
        });
      }
      setSelectedIds([]);
    ...
    <div className="min-h-screen bg-bg-light dark:bg-bg-dark text-slate-800 dark:text-slate-100 flex flex-col transition-colors pb-12">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-8 mt-6 space-y-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="p-4 rounded-2xl glass border border-slate-250 dark:border-slate-800 shadow-lg overflow-x-auto">
                      ...
                      >
                        ...
                        <td className="py-3.5">
                          <span className="font-mono text-[9px] uppercase px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-900 text-slate-500 border border-slate-200 dark:border-slate-800">
                            {comp.ai_classification.type} ({Math.round(comp.ai_classification.confidence * 100)}%)
                          </span>
                        </td>
                        <td className="py-3.5">
          ...
          <AnimatePresence mode="wait">
            {selectedComp ? (
              <motion.div
                ...
                key={selectedComp.id}
                className="rounded-2xl glass border border-slate-250 dark:border-slate-800 shadow-xl p-5 space-y-6 flex flex-col justify-between"
              >
                <div>
                  {/* Title & Close */}
                  <div className="flex justify-between items-start pb-4 border-b border-slate-100 dark:border-slate-850">
                    ...
                    <button
                      ...
                      className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-900 text-slate-400 hover:text-slate-250 transition"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  ...
                  {/* Media viewer */}
                  <div className="mt-4 relative group cursor-pointer overflow-hidden rounded-xl border border-slate-250 dark:border-slate-800" onClick={() => setShowLightbox(true)}>
                    <img
                  ...
                  <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-900 border border-slate-150 dark:border-slate-850 rounded-xl space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-[9.5px] text-slate-450 block">Class Type:</span>
                        <strong className="capitalize text-slate-700 dark:text-slate-200 font-bold">{selectedComp.ai_classification.type}</strong>
                      </div>
                      <div>
                        <span className="text-[9.5px] text-slate-450 block">Confidence:</span>
                        <strong className="font-mono text-slate-700 dark:text-slate-200">{Math.round(selectedComp.ai_classification.confidence * 100)}%</strong>
                      </div>
                      <div>
                        <span className="text-[9.5px] text-slate-450 block">Severity Score:</span>
                        <strong className="font-mono text-slate-700 dark:text-slate-200">{selectedComp.ai_classification.severity_score}/10</strong>
                      </div>
                      <div>
                        <span className="text-[9.5px] text-slate-450 block">Depth Estimation:</span>
                  ...
                  {/* GPS Map Mini Pick */}
                  <div className="h-28 mt-4 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-850">
                    <DynamicMap
                  ...
                  {/* Civilian Mini Card */}
                  {reporter && (
                    <div className="mt-4 p-3 border border-slate-200 dark:border-slate-850 rounded-xl flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <span className="text-[9px] text-slate-450 font-mono block uppercase">
                  ...
                  {/* Budget estimated */}
                  <div className="mt-4 flex justify-between items-center text-xs p-3 bg-secondary-light dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl">
                    <span className="text-slate-400">Projected Repair Cost:</span>
                    <strong className="font-mono text-slate-700 dark:text-slate-200 text-sm">₹{selectedComp.budget_estimated.toLocaleString()}</strong>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-850 space-y-3">
                    ...
                    <div className="space-y-2.5">
                      ...
                      <div className="flex justify-between items-center text-xs">
                        ...
                        <input
                          ...
                          className="w-20 text-center py-1 rounded bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 text-xs font-mono font-bold text-primary focus:outline-none"
                        />
                      </div>
                  ...
                  {(selectedComp.status === "assigned" || selectedComp.status === "in_progress") && (
                    ...
                    >
                      <Check className="w-4.5 h-4.5 mr-1.5" /> Mark Case Resolved (Complete)
                    </button>
                  )}
