...
import { 
  ...
  Percent, ArrowRight, ShieldCheck, Landmark, Sparkles
} from "lucide-react";
...
export default function BudgetAnalytics() {
  ...
  const totalBudget = projects.reduce((sum, p) => sum + p.budget_total, 0) + complaints.reduce((sum, c) => sum + c.budget_estimated, 0);
  ...
  const projectedBudget = complaints.filter(c => c.status !== "resolved" && c.status !== "rejected").reduce((sum, c) => sum + c.budget_estimated, 0);
  ...
  }, 0) + (projects.some(p => p.title.includes("Bulk")) ? 16250 : 0); // fallback default check
  ...
  const districtData = districts.map(dist => {
    ...
    const allocated = distProjs.reduce((sum, p) => sum + p.budget_total, 0) + distComps.reduce((sum, c) => sum + c.budget_estimated, 0);
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `roadguard_budget_report_${new Date().toISOString().split("T")[0]}.csv`);
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-8 mt-6 space-y-6">
          ...
          <div className="p-5 rounded-2xl border border-success/20 bg-success/5 dark:bg-success-light/10 relative overflow-hidden flex justify-between items-center shadow-lg glow-xp">
            ...
            <div className="flex items-center space-x-3.5 pl-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-355 mt-0.5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="p-6 rounded-2xl glass border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col justify-between">
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-850 space-y-1.5 text-xs">
              <div className="flex justify-between items-center">
                <span className="text-slate-400">Current Average Defect Cost:</span>
                <strong className="font-mono text-slate-700 dark:text-slate-250">₹{avgCostPerComplaint.toLocaleString()}</strong>
              </div>
        ...
        <div className="p-6 rounded-2xl glass border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-slate-155 dark:border-slate-800 text-[10.5px] uppercase font-mono text-slate-400 tracking-wider">
                ...
                {districtData.map((d) => {
                  ...
                  return (
                    <tr key={d.district} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition">
                      ...
                      <td className="py-4 font-mono text-slate-600 dark:text-slate-350">
