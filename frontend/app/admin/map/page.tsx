...
import { Complaint, Project } from "@/lib/seedData";
...
export default function AdminMap() {
  ...
  const handlePolygonComplete = (selectedIds: string[], clusterCenter: [number, number]) => {
    ...
    const originalCost = selectedComps.reduce((sum, c) => sum + c.budget_estimated, 0);
  ...
  return (
    <div className="min-h-screen bg-bg-light dark:bg-bg-dark text-slate-800 dark:text-slate-100 flex flex-col transition-colors">
      ...
      <div className="flex-1 flex flex-col lg:flex-row relative">
        ...
        {/* Map Canvas */}
        <div className="flex-1 h-[calc(100vh-130px)] lg:h-[calc(100vh-65px)] relative">
          <DynamicMap
            center={mapCenter}
            zoom={mapZoom}
            pins={mapPins}
          ...
          <div className="absolute bottom-5 left-5 z-[1000] p-4 glass rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col space-y-3 max-w-[200px]">
            ...
            <label className="flex items-center space-x-2 text-xs cursor-pointer">
              ...
              />
              <span className="font-semibold text-slate-650 dark:text-slate-300">Density Heatmap</span>
            </label>
            <label className="flex items-center space-x-2 text-xs cursor-pointer">
              <input
                type="checkbox"
                ...
                className="rounded border-slate-300 text-primary focus:ring-primary"
              />
              <span className="font-semibold text-slate-650 dark:text-slate-300">Bulk Repair Zones</span>
            </label>
          </div>
        </div>

        {/* Sidebar panels */}
        <div className="w-full lg:w-96 glass border-t lg:border-t-0 lg:border-l border-slate-250 dark:border-slate-800 p-5 flex flex-col justify-between max-h-[calc(100vh-130px)] lg:max-h-[calc(100vh-65px)] overflow-y-auto">
            ...
            <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  ...
                  className="py-1.5 px-2.5 rounded-lg text-xs bg-slate-100 dark:bg-slate-900 border border-slate-250 dark:border-slate-800 focus:outline-none"
                >
                      ...
                      <div
                        ...
                        className={`p-2.5 rounded-xl border text-xs cursor-pointer transition ${
                          isSelected
                            ...
                            : "bg-slate-50/50 hover:bg-slate-100/50 dark:bg-slate-900/40 dark:hover:bg-slate-900/80 border-slate-200 dark:border-slate-850"
                        }`}
                      >
                        ...
                        <span className="text-[9.5px] font-mono text-slate-450 block truncate">
                          {comp.address}
                        </span>
                      </div>
                    );
          ...
          <div className="mt-6 pt-4 border-t border-slate-150 dark:border-slate-800">
            {bulkBundle && bulkBundle.show ? (
              <div className="p-4 rounded-xl border border-primary/20 bg-primary/5 dark:bg-primary/10 space-y-3 shadow-md glow-xp">
                ...
                <div className="flex gap-2 pt-1.5">
                  <button
                    ...
                    className="flex-1 py-1.5 rounded-lg border border-slate-350 hover:bg-slate-900 text-[11px] font-bold text-slate-300 transition"
                  >
                    Cancel
                  </button>
                  <button
              ...
              <div className="space-y-3.5 text-xs">
                <div className="pb-2 border-b border-slate-150 dark:border-slate-800 flex justify-between items-center">
              ...
              <div className="p-4 border border-dashed border-slate-350 dark:border-slate-850 rounded-xl flex items-center justify-center text-center text-[11px] text-slate-450">
                <Info className="w-4 h-4 mr-1 text-slate-450" />
