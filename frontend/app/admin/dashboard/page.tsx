...
import {
  ...
  DollarSign, Award, RefreshCw, FileText, ChevronRight, MapPin, Zap,
} from "lucide-react";
import {
  motion,
  AnimatePresence,
...
import { Complaint, CivilianUser, Worker, Project } from "@/lib/seedData";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    setSession(parsed);
    if (parsed.adminRole && parsed.adminRole !== "state") setDistrictFilter(parsed.district);

    const fetchData = async () => {
      try {
        const [comps, civs, wrks, projs] = await Promise.all([
          db.getComplaints(), db.getCivilians(), db.getWorkers(), db.getProjects(),
        ]);
        setComplaints(comps); setCivilians(civs); setWorkers(wrks); setProjects(projs);
    ...
    const unsub = db.subscribe((state) => {
      setComplaints(state.complaints); setCivilians(state.civilians);
      setWorkers(state.workers); setProjects(state.projects);
    });
    return () => unsub();
  }, [router]);
  ...
  const displayWorkers    = userDistrict ? workers.filter(w => w.district === userDistrict) : workers;

  /* KPIs */
  const totalReports    = displayComplaints.length;
  const resolvedReports = displayComplaints.filter(c => c.status === "resolved").length;
  const inProgress      = displayComplaints.filter(c => c.status === "in_progress" || c.status === "assigned").length;
  const activeWorkers   = displayWorkers.filter(w => w.availability === "available").length;
  ...
  const handleCreateBulkProject = async () => {
    ...
    const orig = activeCluster.reduce((s, c) => s + c.budget_estimated, 0);
  ...
  const totalBudget   = projects.reduce((s, p) => s + p.budget_total, 0) + complaints.reduce((s, c) => s + c.budget_estimated, 0);
  const spentBudget   = projects.reduce((s, p) => s + p.budget_spent, 0) + complaints.filter(c => c.status === "resolved").reduce((s, c) => s + (c.budget_actual || 0), 0);
  const pieData = [
              ...
              >
                ...
                <div className="pl-5 relative z-10">
                  ...
                  <p className="text-xs text-slate-600 mt-1 max-w-2xl leading-relaxed">
                    ...
                    <strong>500m radius</strong> in Coimbatore. Single bulk project generates{" "}
                    <strong>25% budget saving (₹16,250 saved)</strong>.
                  </p>
                </div>
