...
import { Complaint, Project, Worker } from "@/lib/seedData";
  ...
  const handleDrop = async (e: React.DragEvent, targetStatus: any) => {
    ...
    try {
      ...
      if (targetStatus === "assigned" && !comp.worker_id) {
        ...
        updates.budget_actual = Math.round(comp.budget_estimated * 0.95);
      }
    ...
    <div className="min-h-screen bg-bg-light dark:bg-bg-dark text-slate-800 dark:text-slate-100 flex flex-col transition-colors pb-12">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-8 mt-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-5 rounded-2xl glass border border-slate-200 dark:border-slate-800 shadow-lg space-y-3 sm:space-y-0">
          ...
          <div className="flex rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-250 dark:border-slate-850 p-1">
            <button
            ...
            {Object.keys(columns).map((statusKey) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl glass border border-slate-200 dark:border-slate-800 border-t-4 ${colColors[statusKey]} flex flex-col space-y-4 min-h-[500px] shadow-md`}
                >
                  {/* Header */}
                  <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-850">
                    ...
                    <span className="font-mono text-xs font-bold px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 text-slate-500">
                      {col.items.length}
                    </span>
                  </div>
                    ...
                    {col.items.map((comp: Complaint) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-850 bg-slate-50/50 hover:bg-slate-100 dark:bg-slate-900/60 dark:hover:bg-slate-900 shadow-sm cursor-grab active:cursor-grabbing hover:border-slate-350 dark:hover:border-slate-800 transition duration-150 space-y-2.5"
                        >
                          ...
                          <div className="flex items-center text-[10px] text-slate-450 truncate">
                          ...
                          <div className="flex justify-between items-center text-[10px] pt-1.5 border-t border-slate-100 dark:border-slate-850">
                            ...
                            <span className="font-mono font-bold text-slate-700 dark:text-slate-200 flex items-center">
                              ...
                              {(comp.budget_estimated / 1000).toFixed(1)}k
                            </span>
                          </div>
            ...
            <div className="space-y-6">
              {projects.length === 0 ? (
                ...
                projects.map((proj) => {
                  ...
                  return (
                    <div key={proj.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center pb-6 border-b border-slate-100 dark:border-slate-850 last:border-b-0">
                      {/* Project info card */}
                      ...
                      <div className="md:col-span-2 space-y-2">
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-slate-450">Project Progress</span>
                      ...
                      <div className="flex flex-row md:flex-col justify-between items-center md:items-end text-xs space-y-1">
                        <div>
                          <span className="text-[10px] text-slate-450 block md:text-right">Project Budget</span>
                        ...
                        <button
                          ...
                          className="py-1 px-3 border border-slate-250 dark:border-slate-800 hover:bg-slate-900 rounded-lg text-[10.5px] font-bold transition flex items-center"
                        >
