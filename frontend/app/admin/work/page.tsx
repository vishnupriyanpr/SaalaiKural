...
import { Complaint, Worker } from "@/lib/seedData";
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
              {unassignedComplaints.length === 0 ? (
                <div className="py-12 border-2 border-dashed border-slate-250 dark:border-slate-850 rounded-xl text-center text-xs text-slate-400">
                  <UserCheck className="w-8 h-8 mx-auto mb-2 text-slate-450" />
                ...
                unassignedComplaints.map((comp) => {
                  ...
                  return (
                    <motion.div
                      ...
                      layoutId={comp.id}
                      className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-850 bg-slate-50/50 hover:bg-slate-100 dark:bg-slate-900/60 dark:hover:bg-slate-900 shadow-sm cursor-grab active:cursor-grabbing hover:border-slate-350 dark:hover:border-slate-800 transition duration-150 space-y-2 relative"
                    >
                      ...
                      <div className="flex justify-between items-center text-[10.5px] pt-1.5 border-t border-slate-100 dark:border-slate-850 text-slate-450">
                        <span className="flex items-center font-mono">
                          <Clock className="w-3 h-3 mr-0.5" />
                          {daysOpen} days old
                        </span>
                        <span className="font-mono text-slate-700 dark:text-slate-300 font-bold">
                          ₹{comp.budget_estimated.toLocaleString()}
                        </span>
                      </div>
                    </motion.div>
                  );
                  ...
                  <div
                    ...
                    className={`p-4 rounded-xl border transition-all duration-200 flex flex-col justify-between space-y-3 ${
                      isRecommended
                        ...
                        : "bg-slate-50/50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-850 hover:border-slate-350 dark:hover:border-slate-800"
                    }`}
                  >
                    <div>
                      {/* Recommendations tag */}
                      ...
                      <div className="flex flex-wrap gap-1 mt-2.5">
                        {worker.skill_tags.map((skill, sIdx) => (
                          <span key={sIdx} className="text-[8.5px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-850">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex justify-between items-center text-[10px] pt-3 border-t border-slate-100 dark:border-slate-850 text-slate-450">
                      <span className="font-mono">
                        Active load: <strong className="font-bold text-slate-700 dark:text-slate-205">{currentLoad} cases</strong>
                      </span>
