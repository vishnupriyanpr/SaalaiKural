...
export default function CivilianDashboard() {
  ...
  const [districtCount, setDistrictCount] = useState(10);
  ...
  useEffect(() => {
    ...
    if (parsed.role !== "civilian") { router.push("/login"); return; }
    setSession(parsed);
    ...
    const unsub = db.subscribe(async (state) => {
      ...
      if (civ) {
        setUser(civ);
        const sorted = state.civilians.filter(c => c.district === civ.district).sort((a, b) => b.points_total - a.points_total);
        setDistrictRank(sorted.findIndex(c => c.id === civ.id) + 1);
        setDistrictCount(sorted.length);
      }
      setComplaints(state.complaints.filter(c => c.civilian_id === parsed.userId));
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col pb-16">
      ...
      <main className="flex-1 max-w-md md:max-w-4xl w-full mx-auto px-4 md:px-8 mt-6">
        <motion.div
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <strong className="text-sm font-extrabold text-slate-800">{user.streak_days} Days in a row</strong>
                </div>
              </div>
