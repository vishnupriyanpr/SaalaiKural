...
|---|---|
| `/webhook-test/roadwatch-chat` | Only while "Test workflow" is active in n8n editor |
| `/webhook/roadwatch-chat` | Only when the workflow is **Published** (Active toggle ON) |
...
| # | Node | Type | Purpose |
|---|---|---|---|
| 1 | Webhook - Chat Input | Webhook (POST) | Receives user messages at `/roadwatch-chat` |
...
| `general` | *(default — no keywords matched)* | "What does RoadWatch do?" |
